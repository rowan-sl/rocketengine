...
#[allow(unused_imports)]
use crate::math::utils::{DEG_TO_RAD, RAD_TO_DEG};
...
impl Quaternion {
    ...
    ///
    /// TODO this also had a version where it took Quaternion, so if that is used add it later
    ...
    pub fn fractional(mut self, a: f32) -> Quaternion {
    ...
    ///
    /// TODO this does not appear to take two vectors, and takes self, so figure out wth is going on here and FIX IT
    pub fn rotation_between_vectors(self, vector: Vec3) -> Quaternion {
        let mut q = Quaternion::new(0.0, vector.x, vector.y, vector.z);
    ...
    pub fn from_euler(euler_angles: Vec3) -> Quaternion {
...
#[test]
fn test_rotate() {
    let q = Quaternion::from_euler(Vec3::new(0.0, 90.0 * DEG_TO_RAD, 0.0));
    println!("{:?}", q);
...
fn test_euler_to_quaternion() {
    let e = Vec3::new(45.0 * DEG_TO_RAD, 45.0 * DEG_TO_RAD, 45.0 * DEG_TO_RAD);
...
fn test_quaternion_to_euler() {
    ...
    let e = q.to_euler();
    let et = Vec3::new(45.0 * DEG_TO_RAD, 45.0 * DEG_TO_RAD, 45.0 * DEG_TO_RAD);
    println!("{:?}", q);
